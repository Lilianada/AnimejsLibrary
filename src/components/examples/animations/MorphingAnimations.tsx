...
const MorphingAnimations = () => {
  ...
  useEffect(() => {
    ...
    if (!anime || !animationRef.current || !anime.animate || !anime.createScope)
      return;

    const target = animationRef.current.querySelector(
      ".morphing-target path",
    ) as SVGPathElement | null;
    if (!target) return;

    scopeRef.current = anime
      .createScope({ root: animationRef.current })
      .add((scope: any) => {
        const animationInstance = anime.animate(target, {
          d: paths.map((p) => p.d),
        ...
        scope.add("setSpeed", (newSpeed: number) => {
          if (
            animationInstance &&
            typeof (animationInstance as any).speed !== "undefined"
          ) {
...
`;
  ...
  return (
    <Card className="shadow-lg border-border">
      ...
      <CardContent>
        <div
          className="mb-6 p-6 bg-muted rounded-lg flex items-center justify-center min-h-[250px]"
          ref={animationRef}
        >
