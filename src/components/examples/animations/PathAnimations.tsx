    ...
    if (animationRef.current) {
      scopeRef.current = createScope({ root: animationRef.current }).add(scope => {
        ...
        const pathAnimation = animate('path', {
          strokeDashoffset: [
            { value: svg.dashOffsetTotal, duration: 0 },
        ...
        const followerAnimation = animate('.path-follower', {
          translateX: {
            value: function(el, i) {
              ...
              const pointAtLength = (t: number) => {
                const point = path ? path.getPointAtLength(t * pathLength) : { x: 0, y: 0 };
                return point;
              };
              
              // Create motion path function
              return function(t: number) {
                const point = pointAtLength(t);
                return point.x;
              };
            },
            duration: 3000,
