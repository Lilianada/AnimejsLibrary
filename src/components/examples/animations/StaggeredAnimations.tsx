...
const StaggeredAnimations = () => {
  ...
  useEffect(() => {
    ...
    if (!anime || !animationRef.current || !anime.animate || !anime.stagger)
      return;

    const targets = animationRef.current?.querySelectorAll(".stagger-target");
    if (targets && targets.length > 0) {
      animationInstanceRef.current = anime.animate(targets, {
        translateY: [-20, 0],
        opacity: [0, 1],
        scale: [0.8, 1],
        duration: 600,
    ...
    return () => {
      if (
        ...
        typeof animationInstanceRef.current.pause === "function"
      ) {
...
`;
  ...
  return (
    <Card className="shadow-lg border-border">
      ...
      <CardContent>
        <div
          className="mb-6 p-6 bg-muted rounded-lg flex items-center justify-center min-h-[200px]"
          ref={animationRef}
        >
          <div className="grid grid-cols-4 gap-4">
            {[...Array(12)].map((_, i) => (
              <div
                key={i}
                className="stagger-target h-10 w-10 bg-accent rounded-md opacity-0"
              ></div>
            ))}
          </div>
        </div>
